...
class Main extends Component<MyProps, MyState> {
  updateDimensions = () => {
    ...
    this.setState({
      x: Math.floor(
        (window.innerWidth - 40) / (window.innerWidth > 400 ? 20 : 10)
      ),
      y: window.innerWidth > 400 ? 20 : 40,
      tableArr: Array.from(Array(20), () =>
        Array.from(
          Array(
            Math.floor(
              (window.innerWidth - 40) / (window.innerWidth > 400 ? 20 : 10)
  ...
  drawPuzzle() {
    ...
    if (!stop) {
      setTimeout(() => {
        const newTableArr = Array.from(Array(this.state.y), () =>
          Array.from(Array(this.state.x), () => "black")
        );
